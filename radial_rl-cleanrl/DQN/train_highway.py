...
#     next_state = next_state / 255
    ...
    # q_values      = curr_model(obs)
    # next_q_values = curr_model(next_obs)
    # next_q_state_values = target_model(next_obs) 
...
#     upper, lower = network_bounds(curr_model.network, state.permute((0, 3, 1, 2)), epsilon)
    upper, lower = network_bounds(curr_model.network, torch.reshape(state, (-1, np.array(curr_model.env.observation_space.shape).prod())), epsilon)
    onehot_labels = torch.zeros(upper.shape).to(device)
    onehot_labels[range(torch.reshape(state, (-1, np.array(curr_model.env.observation_space.shape).prod())).shape[0]), action] = 1
#     onehot_labels[range(state.permute((0, 3, 1, 2)).shape[0]), action] = 1
...
def train(current_model, target_model, env, optimizer, args):
    ...
    replay_buffer = ReplayBuffer(args.buffer_size, env.observation_space, env.action_space, device=device, optimize_memory_usage=True)
    start = time.time()
...
#         action = [current_model.act(obs, action_epsilon)]
        action = current_model.act(obs, action_epsilon)
        # print(action)
        ...
        replay_buffer.add(state, next_state, action.cpu(), reward.cpu(), done)
